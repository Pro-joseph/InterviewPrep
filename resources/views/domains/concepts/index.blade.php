<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <div class="breadcrumb mb-2">
                    <a href="{{ route('dashboard') }}">Dashboard</a>
                    <span class="breadcrumb-separator">/</span>
                    <a href="{{ route('domains.index') }}">Domaines</a>
                    <span class="breadcrumb-separator">/</span>
                    <span class="text-primary">{{ $domain->name }}</span>
                </div>
                <h2 class="font-display text-2xl font-semibold text-primary flex items-center gap-3">
                    <div class="w-3 h-3 rounded-full" style="background: {{ $domain->color }};"></div>
                    {{ $domain->name }}
                </h2>
            </div>
            <a href="{{ route('domains.concepts.create', $domain) }}" class="btn btn-primary">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                </svg>
                Nouveau Concept
            </a>
        </div>
    </x-slot>

    <div class="py-6">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="filters-bar">
                <div class="filter-group">
                    <span class="filter-label">Statut:</span>
                    <select class="form-select text-sm" style="padding: 8px 32px 8px 12px;">
                        <option value="">Tous</option>
                        <option value="a_revoir">À revoir</option>
                        <option value="en_cours">En cours</option>
                        <option value="maitrise">Maîtrisé</option>
                    </select>
                </div>

                <div class="filter-group">
                    <span class="filter-label">Difficulté:</span>
                    <select class="form-select text-sm" style="padding: 8px 32px 8px 12px;">
                        <option value="">Toutes</option>
                        <option value="junior">Junior</option>
                        <option value="mid">Mid</option>
                        <option value="senior">Senior</option>
                    </select>
                </div>
            </div>

            @if($concepts->isEmpty())
                <div class="empty-state">
                    <div class="empty-state-icon">
                        <svg class="w-full h-full" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                        </svg>
                    </div>
                    <h3 class="empty-state-title">Aucun concept</h3>
                    <p class="empty-state-text">Ajoute ton premier concept dans ce domaine</p>
                    <button @click="$dispatch('open-modal', 'create-concept')" class="btn btn-primary">
                        Ajouter un concept
                    </button>
                </div>
            @else
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    @foreach($concepts as $concept)
                        <div class="concept-card">
                            <div class="flex items-start justify-between mb-3">
                                <a href="{{ route('domains.concepts.show', [$domain, $concept]) }}" class="flex-1">
                                    <h3 class="font-semibold text-primary hover:text-primary transition-colors">
                                        {{ $concept->title }}
                                    </h3>
                                </a>
                                <div class="dropdown" x-data="{ open: false }">
                                    <button @click.stop="open = !open" class="btn btn-ghost btn-icon">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 5v.01M12 12v.01M12 19v.01M12 6a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2z" />
                                        </svg>
                                    </button>
                                    <div x-show="open" @click.outside="open = false" class="dropdown-menu" style="display: none;">
                                        <a href="{{ route('domains.concepts.edit', [$domain, $concept]) }}" class="dropdown-item">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                            </svg>
                                            Modifier
                                        </a>
                                        <form action="{{ route('domains.concepts.destroy', [$domain, $concept]) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="dropdown-item text-red-500 w-full text-left">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                                </svg>
                                                Supprimer
                                            </button>
                                        </form>
                                    </div>
                                </div>
                            </div>

                            <div class="flex items-center gap-3 mb-4">
                                <span class="badge badge-{{ $concept->difficulty }}">
                                    {{ match($concept->difficulty) {
                                        'junior' => 'Junior',
                                        'mid' => 'Mid',
                                        'senior' => 'Senior'
                                    } }}
                                </span>
                                <span class="badge badge-{{ match($concept->status) {
                                    'a_revoir' => 'review',
                                    'en_cours' => 'progress',
                                    'maitrise' => 'mastered'
                                } }}">
                                    {{ match($concept->status) {
                                        'a_revoir' => 'À revoir',
                                        'en_cours' => 'En cours',
                                        'maitrise' => 'Maîtrisé'
                                    } }}
                                </span>
                            </div>

                            <div class="flex items-center justify-between pt-3 border-t border-default">
                                <div class="flex items-center gap-2">
                                    <span class="text-xs text-muted">Statut:</span>
                                    <select
                                        class="status-select status-select-{{ match($concept->status) { 'a_revoir' => 'review', 'en_cours' => 'progress', 'maitrise' => 'mastered' } }}"
                                        onchange="updateStatus({{ $concept->id }}, this.value)"
                                    >
                                        <option value="a_revoir" {{ $concept->status === 'a_revoir' ? 'selected' : '' }}>À revoir</option>
                                        <option value="en_cours" {{ $concept->status === 'en_cours' ? 'selected' : '' }}>En cours</option>
                                        <option value="maitrise" {{ $concept->status === 'maitrise' ? 'selected' : '' }}>Maîtrisé</option>
                                    </select>
                                </div>
                                <a href="{{ route('domains.concepts.show', [$domain, $concept]) }}" class="text-sm text-primary hover:underline">
                                    Voir détail →
                                </a>
                            </div>
                        </div>
                    @endforeach
                </div>

                <div class="mt-6">
                    {{ $concepts->links() }}
                </div>
            @endif
        </div>
    </div>

    <x-modal name="create-concept">
        <form method="POST" action="{{ route('domains.concepts.store', $domain) }}" class="p-6">
            @csrf
            <h3 class="font-display text-xl font-semibold text-primary mb-6">Nouveau Concept</h3>

            <div class="form-group">
                <label for="title" class="form-label">Titre</label>
                <input
                    type="text"
                    id="title"
                    name="title"
                    class="form-input"
                    placeholder="Ex: Eloquent N+1 Problem"
                    required
                />
                @error('title')
                    <p class="form-error">{{ $message }}</p>
                @enderror
            </div>

            <div class="form-group">
                <label for="explanation" class="form-label">Explication</label>
                <textarea
                    id="explanation"
                    name="explanation"
                    rows="6"
                    class="form-textarea"
                    placeholder="Explique ce concept dans tes propres mots..."
                    required
                ></textarea>
                @error('explanation')
                    <p class="form-error">{{ $message }}</p>
                @enderror
            </div>

            <div class="grid grid-cols-2 gap-4">
                <div class="form-group">
                    <label for="difficulty" class="form-label">Difficulté</label>
                    <select id="difficulty" name="difficulty" class="form-select" required>
                        <option value="junior">Junior</option>
                        <option value="mid">Mid</option>
                        <option value="senior">Senior</option>
                    </select>
                </div>

                <div class="form-group">
                    <label for="status" class="form-label">Statut</label>
                    <select id="status" name="status" class="form-select" required>
                        <option value="a_revoir">À revoir</option>
                        <option value="en_cours">En cours</option>
                        <option value="maitrise">Maîtrisé</option>
                    </select>
                </div>
            </div>

            <div class="flex justify-end gap-3 mt-6">
                <button type="button" x-on:click="$dispatch('close')" class="btn btn-ghost">Annuler</button>
                <button type="submit" class="btn btn-primary">Créer</button>
            </div>
        </form>
    </x-modal>
</x-app-layout>